...
//組件Html渲染
function module_Introduction(num, title, id) {
    let defaultTitle = title === undefined ? '文字區塊' + num : title
    let defaultId = ''
    if (id) {
        defaultId = 'data-introduction-id="' + id + '"'
    }
    let content = '<div class="module_box module_box_Introduction" id="Introduction_' + num + '"' + defaultId + 'data-introduction-sort="' + num + '">\n' +
...
//組件-ADD
function module_add(name, id, title, content) {
    //判斷新增之組件
    if (name == 'Introduction') {
        //創建新組件
        let Introduction_id = 'Introduction_editor_' + Introduction_num;
        $('#module').append(module_Introduction(Introduction_num, title, id));
        //組件CKEDITOR渲染
        CKEDITOR.replace(Introduction_id);
        //甜素資料
        if (content) {
            CKEDITOR.instances[Introduction_id].setData(content);
        }
        Introduction_num += 1;
...
function module_delete(id, categories_id) {
    ...
    if (yes) {
        ...
        categoriesArr = categoriesArr.filter(function (e) {
            return e !== 59
        })
        //完成通知
